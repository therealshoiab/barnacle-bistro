...
export default function Navbar({ 
  ...
  return (
    <nav 
      className={`navbar-container ${isScrolled ? 'nav-scrolled' : ''}`}
      style={{
        position: 'fixed',
        ...
        boxShadow: isScrolled
          ? 'rgba(0, 0, 0, 0.12) 0px 10px 30px, rgba(212, 175, 55, 0.04) 0px 4px 16px'
    ...
    >
          ...
          {navLinks.map((link) => (
            <button
              ...
              style={{
                ...
                borderRadius: '12px',
                backgroundColor: currentPage === link.key ? 'var(--accent-glow)' : 'transparent',
                transition: 'all 0.2s'
              }}
            >
              {link.label}
      ...
      {/* CSS rules for navigation rendered cleanly */}
      <style>{`
        .nav-desktop-links {
          display: flex;
          align-items: center;
